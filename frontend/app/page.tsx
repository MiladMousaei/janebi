...
export default async function Home() {
  ...
  return <SiteChrome><main className="homePage">
    ...
    {offers.length > 0 && <section className="flashSale shell"><aside><span><Zap size={16}/> پیشنهاد شگفت‌انگیز</span><p>فرصت باقی‌مانده تا پایان تخفیف</p><div className="countdown"><b>۱۲<small>ثانیه</small></b><i>:</i><b>۴۵<small>دقیقه</small></b><i>:</i><b>۳۰<small>ساعت</small></b></div><a href="/shop?discount=true">مشاهده همه</a></aside><div className="flashProducts">{offers.map((p,index)=><ProductCard key={p.id} product={p} index={index}/>)}</div></section>}
